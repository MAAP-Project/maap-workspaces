...
"""
    ...
    with open("TempCudaTest.Dockerfile", "w") as f:
    ...
    try:
        ...
        if os.path.exists("TempCudaTest.Dockerfile"):
            os.remove("TempCudaTest.Dockerfile")
...
if __name__ == "__main__":
    ...
    if not tags_to_test:
        print("❌ No valid candidate tags found older than cut off date")
    else:
        print(f"✅ Found {len(tags_to_test)} candidate images. Starting sequence...")
        print("tags to test are ")
        print(tags_to_test)
